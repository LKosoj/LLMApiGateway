...
Браузерный вход теперь идёт через отдельную страницу `/auth/login`. Если открыть `/` без авторизации, gateway перенаправит на `/auth/login`; если валидная авторизация уже есть, `/` перенаправит на `/v1/ui/usage-stats`. После успешного входа сервер создаёт долговечную подписанную HttpOnly cookie с `SameSite=Lax`; по умолчанию cookie также получает флаг `Secure`, и для локального HTTP-окружения его нужно явно отключить через `SESSION_COOKIE_SECURE=false`. Браузерный UI использует уже эту cookie, а не `sessionStorage`; cookie сохраняется между перезапусками браузера и самого gateway, пока вы явно не выйдете или не изменится `GATEWAY_API_KEY`. Для SDK и прямых API-вызовов по-прежнему поддерживается заголовок `Authorization: Bearer <GATEWAY_API_KEY>`, а для `/v1/messages` дополнительно принимается `X-Api-Key: <GATEWAY_API_KEY>` вместе со стандартным заголовком `anthropic-version` официального SDK. Gateway разбирает Bearer-заголовок строго как `scheme + token`, разделённые пробелами, принимает `Bearer` без учёта регистра, возвращает `401` для некорректного формата или отсутствующей авторизации и возвращает `403` только в случае, когда формат заголовка корректен, но значение токена неверно. Без API-ключа доступны только `/health`, `/healthz`, `/auth/login`, `/` как redirect-entrypoint и статические assets под `/static/`; `POST /auth/login` при этом участвует в brute-force блокировке, описанной ниже. HTML-страницы без cookie или Bearer-токена перенаправляются на login, а API-эндпоинты без валидной авторизации продолжают отвечать `401/403`. Незарегистрированные маршруты по-прежнему отдают стандартный `404`.
...
Если вы запускаете gateway через `docker-compose.yml`, смонтированные файлы `providers.json`, `models_fallback_rules.json`, `models_operation_rules.json`, `models_fusion_rules.json`, `models_router_rules.json` и `models_model_rules.json` доступны на запись внутри контейнера. Изменения, внесённые в веб-редакторе, сохраняются обратно в файлы на хосте, включая Fusion-ансамбли и Router-модели.
...
Ограничения: Fusion-модели не поддерживают `stream: true` (возвращается `400`) и не допускают вложенности — вызов Fusion-модели изнутри другой Fusion-панели отклоняется (`400`). Файл задаётся переменной `FUSION_RULES_FILENAME` (по умолчанию `models_fusion_rules.json`); пустой файл или его отсутствие просто означает «нет Fusion-моделей». Структурированный API вкладки — `GET`/`POST /v1/config/fusion-rules/structured`.
...
Если selector вернул невалидный JSON или `candidate_id`, которого нет в allowlist, gateway отвечает явной ошибкой `502` и не выбирает запасную цель молча. Router-модели не поддерживают `stream: true` (возвращается `400`). Настройка доступна во вкладке **Router** редактора конфигурации через Structured API `GET`/`POST /v1/config/router-rules/structured`. Файл задаётся переменной `ROUTER_RULES_FILENAME` (по умолчанию `models_router_rules.json`); пустой файл или его отсутствие означает «нет Router-моделей».
...
Единицы измерения — **USD за 1 миллион токенов**. После сохранения конфигурация перезагружается без рестарта сервера. Если upstream-ответ не содержит `usage.cost`, gateway рассчитывает `cost` локально по текущим ставкам модели и фактическим prompt/completion-токенам; если upstream явно вернул `usage.cost`, это значение остаётся источником истины.
...
> `requirements.txt`, `requirements-dev.txt` и `requirements-research.txt` — сгенерированные файлы, не правьте их вручную.
...
> pip-compile pyproject.toml --constraint=requirements.txt --constraint=requirements-dev.txt --extra=research -o requirements-research.txt --strip-extras
> ```
...
При использовании `docker-compose.yml` файлы конфигурации `providers.json`, `models_fallback_rules.json`, `models_operation_rules.json`, `models_fusion_rules.json`, `models_router_rules.json` и `models_model_rules.json` монтируются с доступом на запись, поэтому веб-редактор может сохранять изменения прямо из контейнера, включая Fusion-ансамбли и Router-модели. Если `models_operation_rules.json` или `models_fusion_rules.json` отсутствуют в контейнере, entrypoint создаёт безопасные пустые файлы (`{}` и `[]`) без фейковых downstream routes.
...
Скрипт использует текущий репозиторий как `WorkingDirectory`, запускает gateway через локальный `.venv/bin/python` и создаёт unit `llm-gateway.service` в `/etc/systemd/system`, если его ещё нет. Новый unit создаётся с `UMask=0007`, чтобы runtime-файлы оставались доступны группе сервиса. Если unit уже существует, скрипт не перезаписывает его и просто делает `daemon-reload` + `restart`.
