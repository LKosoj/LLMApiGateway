...
| `YOUTUBE_PROXY_URL` | Прокси (`http://`, `https://`, `socks5://`, `socks5h://`) для скачивания субтитров YouTube в `/v1/web/read` и `/v1/web/research`. YouTube блокирует свой endpoint субтитров для IP дата-центров, поэтому при блокировке запрос без прокси завершается ошибкой `503` вместо подмены содержимым HTML-страницы видео. Если переменная не задана, субтитры запрашиваются напрямую с IP gateway. Для SOCKS-прокси, резолвящих домены на своей стороне, указывайте схему `socks5h://`. | disabled |
