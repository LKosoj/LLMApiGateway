...
Селектор моделей секции **Chat** включает обычные fallback-модели и Fusion-ансамбли (`models_fusion_rules.json`); Fusion-модели помечены суффиксом `· Fusion`. При выборе Fusion-модели появляется чекбокс **include_details** (по умолчанию включён) — он передаётся в запрос как `fusion.include_details`. Если в ответе есть поле `fusion`, под текстом ассистента раскрывается блок «Fusion ensemble» с атрибуцией main/judge, структурным разбором судьи (agreements/disputes/per-model insights/blind spots) и ответами панели (контент панели показывается только при включённом `include_details`, ошибочные панелисты помечаются отдельно).
