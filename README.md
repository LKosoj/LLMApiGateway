...
- **Временный cooldown модели**: если chat-модель в fallback-цепочке исчерпала свои попытки из-за временного сбоя доступности (`429`, `5xx`, timeout/connect, `overloaded`, `rate_limit`, `try again later`), gateway помечает пару `provider/model` на 10 минут и в следующих запросах пропускает её без downstream-вызова. Метка хранится в памяти процесса и сбрасывается при рестарте.
...
- **Рейтинг бесплатных OpenRouter-моделей**: если настроен официальный провайдер `openrouter` и его API-ключ, gateway каждые 8 часов анализирует бесплатные text-модели OpenRouter. Для scoring можно указать несколько OpenRouter-ключей через запятую: при каждом запросе к OpenRouter выбирается следующий непустой ключ по round-robin. При изменении списка eligible-моделей выполняется полный скоринг (`metadataScore + healthScore + latencyScore + liteEvalScore - instabilityPenalty`), а если список не изменился — обновляются только health/latency. Lite eval проверяет instruction following, tool-call JSON, Python code с unit tests, параметризованную арифметику и factual QA. Результат отображается в read-only вкладке **OpenRouter Free** в `/v1/ui/rules-editor`; над рейтингом показана краткая расшифровка scoring-метрик. Маршрутизация моделей пока не меняется автоматически.
- **Ручной eval fallback-моделей**: во вкладке **Fallback Eval** можно кнопкой запустить такой же lite eval по уникальным `provider/model` из настроенных `fallback_models` и `context_overflow_fallback`. Повторяющиеся цели схлопываются и показывают все gateway-модели, которые на них ссылаются. Счёт считается как `healthScore + latencyScore + liteEvalScore - instabilityPenalty`, `metadataScore` равен 0, потому что каталоги разных провайдеров здесь не нормализуются. Native Anthropic routes помечаются как `unsupported_provider_type`; правила маршрутизации eval не меняет.
- **Virtual API keys**: помимо master-ключа `GATEWAY_API_KEY` теперь можно заводить отдельные виртуальные ключи через UI `/v1/ui/api-keys` (доступно только мастеру). Каждый ключ хранится в `db/api_keys.db` в открытом виде и имеет свой бюджет в USD, лимиты RPM/TPM и опциональный whitelist `allowed_models`. При исчерпании бюджета gateway отвечает `429`, при запрете модели — `403`. Виртуальные ключи могут авторизоваться через `Authorization: Bearer <lgk_...>` или `X-Api-Key`, и в UI видят только свою статистику, без доступа к настройкам. Master-админ на странице ключей может открыть usage-статистику конкретного ключа. Если `budget_usd` не задан, бюджет не отслеживается.
...
## Эндпоинты gateway
  ...
  - `/v1/chat/completions` - OpenAI-совместимый API, который маршрутизирует вызовы к другим провайдерам и использует fallback в случае ошибок.
...
- **OpenRouter Free**: read-only рейтинг бесплатных text-моделей OpenRouter. Lite eval включает instruction following, tool-call JSON, Python code с unit tests, параметризованную арифметику и factual QA. Вкладка показывается только если настроен официальный провайдер `openrouter` и его API-ключ; правила маршрутизации она автоматически не меняет.
...
**Скоринг настроенных fallback-моделей** находится во вкладке **Fallback Eval**. Он запускается вручную кнопкой `Run Eval`, берёт все `fallback_models` и `context_overflow_fallback` из `models_fallback_rules.json`, схлопывает повторы до уникальных пар `provider/model` и показывает, какие gateway-модели ссылаются на каждую цель. Для OpenAI-compatible провайдеров eval отправляет запросы напрямую в `{baseUrl}/chat/completions`, учитывает `custom_headers` (кроме security-заголовков), `custom_body_params`, `providers_order`, provider-specific proxy clients и round-robin выбор ключей, если в `apikey` указано несколько ключей через запятую. Native Anthropic routes не деградируют через другой путь и явно получают статус `unsupported_provider_type`.
Итоговый score для Fallback Eval считается как `healthScore + latencyScore + liteEvalScore - instabilityPenalty`; `metadataScore` всегда равен 0, потому что каталоги разных провайдеров не нормализуются. `healthScore` берётся из короткого probe-запроса `Reply with exactly OK`, `latencyScore` — из скорости этого probe, `liteEvalScore` — из instruction following, tool-call JSON, Python unit test, параметризованной арифметики и factual QA. Результат хранится как runtime snapshot, доступен через `GET /v1/fallback-model-evals`, запускается через `POST /v1/fallback-model-evals/run` и не меняет правила маршрутизации автоматически.
...
Во вкладке **Latest Usage Records** колонка `Timestamp` показывает дату и время запроса в том же формате, что и **Fallback Chains**: `YYYY-MM-DDTHH:MM:SS`, без микросекунд. Запущенные запросы показываются вверху таблицы и выделяются цветом; после завершения они заменяются обычными usage-записями.
...
Владельцы виртуальных ключей при логине в UI видят только `/v1/ui/usage-stats` со статистикой по собственным вызовам; вкладки **Fallback Analytics**, **Rules Editor** и **API Keys** для них скрыты, а master-only API-эндпоинты отвечают `403`. Списание `spent_usd` привязано к `api_key_id` и идёт через тот же `WriteBatcher`, что и основная таблица usage.

Master-админ на странице `/v1/ui/api-keys` может нажать **Usage** у конкретного ключа и увидеть сводку за последние 12 месяцев вместе с последними usage-записями только этого ключа. Для автоматизации master может передавать query-параметр `api_key_id` в `GET /v1/api/usage-stats/{period}` и `GET /v1/api/usage-records`; для владельцев виртуальных ключей этот параметр игнорируется, и статистика остаётся ограниченной их собственным ключом.

CRUD над виртуальными ключами доступен через Admin API (master-only):
- `GET /v1/admin/api-keys` — список всех ключей вместе со значениями `api_key`, `spent_usd`, `last_used_at`.
- `POST /v1/admin/api-keys` — создать новый ключ; ответ содержит `api_key`, полный ключ также виден master-админу в списке и карточке редактирования.
- `PATCH /v1/admin/api-keys/{id}` — частичное обновление полей ключа, включая `reset_spent: true` для обнуления счётчика.
...
- **Chains** — детальные цепочки фолбэков по конкретным запросам. Показываются только запросы, в которых было 2+ попытки. Каждую цепочку можно раскрыть и увидеть последовательность: какие провайдеры были попробованы, причину отказа каждого, расширенное сообщение ошибки для generic upstream-ошибок и transport-ошибок (`ReadTimeout`, `ConnectTimeout`, `ConnectError`) с краткой сводкой payload, и сколько времени заняла каждая попытка (с визуальной шкалой длительности).

Данные о фолбэках хранятся в таблице `fallback_events` в той же БД (`tokens_usage.db`) и подлежат той же автоочистке (90 дней).
...
| `ZAI_API_KEY` | API-ключ Z.AI для встроенных адаптеров `/v1/web/search` и `/v1/web/read` (Z.AI Reader). Можно указать несколько ключей через запятую: при каждом вызове Z.AI gateway выбирает следующий непустой ключ по round-robin. Если не задан, адаптер Z.AI пропускается. | disabled |
...
[
    {
        "openrouter":
        {
            ...
            "apikey" : "${APIKEY_OPENROUTER}"  // явная ссылка на переменную окружения с API-ключом провайдера
        }
    },
    {
        "nebius":
...
7.  Если выбранная модель исчерпала свои попытки из-за временного сбоя доступности (`429`, `5xx`, timeout/connect, `overloaded`, `rate_limit`, `try again later`), gateway ставит пару `provider/model` в in-memory cooldown на 10 минут.
8.  Если выбранная модель завершилась другой ошибкой, либо `context_overflow_fallback` тоже не сработал, gateway пытается вызвать следующую модель в последовательности, пока одна из них не отработает успешно. Модели в активном cooldown пропускаются без downstream-вызова.
