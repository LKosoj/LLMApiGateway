...
Данные о фолбэках и upstream analytics хранятся в таблице `fallback_events` в той же БД (`tokens_usage.db`) и подлежат той же автоочистке (90 дней). Успешные usage-записи дополнительно сохраняют `upstream_key_fingerprint` и входящий `X-Title`, чтобы можно было сверять токены и стоимость с выбранным upstream-ключом или клиентским названием без раскрытия самого секрета.
