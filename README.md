...
- **Cooldown при отказе в доступе (`401`/`403`)**: отказ в доступе — отозванный или неверный ключ, модель вне тарифа, закрытый регион — не относится к классу временных сбоев, поэтому penalty за него не начисляется. Тем не менее gateway ставит паре `(provider, model, upstream key)` тот же фиксированный cooldown на 10 минут и помечает health ключа как `invalid`: без cooldown отвергнутая цель остаётся первой в fallback-цепочке и вызывается заново на каждом запросе. На вкладке **Statistics → Fallback → Chains** такая цепочка сразу, без раскрытия карточки, показывает плашку «Доступ запрещён — модель в кулдауне: `provider/model`», а бейдж самой попытки (`401`/`403`) подсвечивается отдельным цветом. Cooldown отсчитывается от последней неудачной попытки, а любой успешный ответ того же `(provider, model, upstream key)` снимает его немедленно — включая успех локального retry внутри того же запроса, который выполняется тем же ключом и поэтому cooldown не проверяет.
...
- **Empty-text-block shim**: перед отправкой downstream gateway убирает из сообщений пустые текстовые блоки `{"type": "text", "text": ""}`. Клиенты, приводящие каждое сообщение к OpenAI content parts, превращают в такой блок ассистентский ход, состоящий только из `tool_calls`, а строгие провайдеры (например `api.kimi.com`) отвечают на это `400 text content is empty`. Блоки с пробельным текстом не трогаются — они принимаются upstream. Сообщение, у которого после чистки не осталось ни одного блока, отправляется без изменений, если у него нет `tool_calls`: некорректный запрос должен отклонить провайдер, а не молча исправить gateway.
- **Выученное понижение forced `tool_choice`**: некоторые провайдеры принимают `tools`, но отклоняют требование обязательно вызвать инструмент — например compatible-mode эндпоинт alibaba отвечает `400 invalid_parameter_error` на `tool_choice: "required"` (и на объект с именем функции), пока модель работает в thinking mode, который у неё нельзя выключить. Получив такой отказ, gateway один раз повторяет ту же модель с `tool_choice: "auto"`, не тратя retry, и запоминает отказ для пары `(провайдер, модель)`: последующие запросы уходят сразу с `auto`, без заведомо провальной попытки. Понижение ослабляет запрос клиента, поэтому применяется только против upstream, который сам отклонил жёсткую форму; каждое применение пишется в лог, а выученный список виден в `GET /api/upstream-status` (поле `forced_tool_choice_unsupported`). Список живёт в памяти процесса и очищается при рестарте — тогда первый же отказ выучивается заново.
- **Синтез терминального `data: [DONE]` в стриме**: часть OpenAI-совместимых провайдеров (например `minimax`) закрывает SSE-поток сразу после финального чанка с `finish_reason` и не присылает маркер `data: [DONE]`. Для gateway отсутствие терминала означает пустой или оборванный ответ, поэтому такой стрим завершался ошибкой `protocol` уже после того, как весь текст ушёл клиенту (в браузере это выглядело как `Streaming error: Load failed` после полностью отрисованного ответа). Теперь gateway дописывает маркер сам — но только если upstream действительно завершил генерацию, то есть прислал чанк с непустым `finish_reason`; каждый такой случай пишется в лог предупреждением. Поток, оборванный без `finish_reason`, по-прежнему считается сбоем, поэтому усечённый ответ никогда не выдаётся за полный.
- **Обрезка «мусора» после терминала стрима**: зеркальная проблема — часть провайдеров, наоборот, дописывает байты уже после маркера `data: [DONE]` (например `anymodel` на моделях `am/*` присылает маркер дважды подряд). По спецификации SSE поток заканчивается на терминале, поэтому всё, что идёт после него, gateway расценивал как нарушение протокола и отвечал `502 Upstream response failed protocol validation` — снова уже после того, как клиент получил полный ответ. Теперь такие байты не пересылаются клиенту, а факт отброса пишется в лог предупреждением с количеством байт. Само содержимое ответа не меняется: обрезается только хвост за терминалом.
...
## Эндпоинты gateway
  ...
  - Готовый отчёт deep research доставляется, даже если результат не помещается в лимит межпроцессного кадра (16 МБ): дочерний процесс последовательно отбрасывает тела прочитанных источников, затем собранный контекст и дерево исследования, пишет об этом WARNING в лог и всё равно отправляет отчёт. `output`, `source_urls`, `images` и стоимость сохраняются. Сбой завершения дочернего процесса после готового отчёта тоже больше не превращает успешный запуск в 503.
...
На desktop кнопка в верхней части общей боковой панели сворачивает её с 240 до 72px, оставляя только пиктограммы, и разворачивает обратно. Выбранный режим сохраняется между страницами; в компактном виде названия доступны как подсказки, а язык, тема и выход остаются доступны как иконки. Мобильный drawer всегда показывает полные подписи.
...
Браузерный вход теперь идёт через отдельную страницу `/auth/login`. Если открыть `/` без авторизации, gateway перенаправит на `/auth/login`; если валидная авторизация уже есть, `/` перенаправит на `/v1/ui/overview` — это стартовая страница после входа для обеих ролей. После успешного входа сервер создаёт долговечную подписанную HttpOnly cookie с `SameSite=Lax`; по умолчанию cookie также получает флаг `Secure`, и для локального HTTP-окружения его нужно явно отключить через `SESSION_COOKIE_SECURE=false`. Браузерный UI использует уже эту cookie, а не `sessionStorage`; cookie сохраняется между перезапусками браузера и самого gateway, пока вы явно не выйдете или не изменится `GATEWAY_API_KEY`. Подписывается cookie отдельным случайным секретом, который gateway создаёт при первом запуске и хранит в `db/session_secret.json` (каталог задаётся `GATEWAY_DB_DIR`); сам `GATEWAY_API_KEY` в подпись не попадает, поэтому по выданной cookie нельзя офлайн подобрать мастер-ключ. Файл лежит рядом с остальным состоянием в `db/` и переживает рестарт вместе с ним; при смене `GATEWAY_API_KEY` секрет автоматически перевыпускается — именно так смена ключа и разлогинивает всех. При первом запуске после обновления секрета ещё нет, поэтому выданные ранее cookie один раз становятся недействительными. Для SDK и прямых API-вызовов по-прежнему поддерживается заголовок `Authorization: Bearer <GATEWAY_API_KEY>`, а для `/v1/messages` дополнительно принимается `X-Api-Key: <GATEWAY_API_KEY>` вместе со стандартным заголовком `anthropic-version` официального SDK. Gateway разбирает Bearer-заголовок строго как `scheme + token`, разделённые пробелами, принимает `Bearer` без учёта регистра, возвращает `401` для некорректного формата или отсутствующей авторизации и возвращает `403` только в случае, когда формат заголовка корректен, но значение токена неверно. Без API-ключа доступны только `/health`, `/auth/login`, `/` как redirect-entrypoint и статические assets под `/static/`; `POST /auth/login` при этом участвует в brute-force блокировке, описанной ниже. HTML-страницы без cookie или Bearer-токена перенаправляются на login, а API-эндпоинты без валидной авторизации продолжают отвечать `401/403`. Незарегистрированные маршруты по-прежнему отдают стандартный `404`.

Общая навигация строится после одного запроса текущей роли. На экранах от 1024px и шире она отображается как постоянная левая боковая панель шириной ~240px: master видит двенадцать UI-разделов (**Overview**, **Activity**, **Docs**, **Analytics (устар.)**, **Quota**, **Rules Editor**, **Playground**, **API Keys**, **Rejections**, **Проверка транслятора**, **Pricing**, **Free LLM Catalog**), virtual key — только пять: **Overview**, **Activity**, **Docs**, **Quota** и **Free LLM Catalog**. Внизу боковой панели над данными пользователя находится общий блок настроек с пиктограммой языка, выбором локали и кнопкой светлой/тёмной/системной темы; отдельной панели в правом верхнем углу нет. Ниже показывается текущая роль (`Master`/`User`), а для virtual key — ещё и имя ключа из `/auth/me`, и кнопка выхода, которая вызывает `POST /auth/logout` и возвращает на страницу входа. На экранах менее 1024px та же панель превращается в выдвижной drawer: кнопка-гамбургер (44×44) в шапке открывает его поверх содержимого; закрыть можно клавишей `Escape`, кликом вне панели или кликом по пункту меню, фокус на время открытия удерживается внутри drawer. Список разделов поддерживает навигацию стрелками `↑`/`↓`, а также `Home`/`End`. На странице входа русский или английский язык можно выбрать до авторизации. После загрузки локали подписи переключателя темы и версия продукта обновляются на выбранном языке без повторных запросов `/auth/me` или `/health`. Ошибки `POST /auth/login` дополнительно содержат стабильный `error.code` (`auth_invalid_api_key`, `auth_invalid_request`, `auth_rate_limited` или `auth_unavailable`), сохраняя прежние HTTP status и верхнеуровневый `detail`. Группы вкладок поддерживают стрелки, `Home`/`End` и ручную активацию через `Enter`/`Space`.
...
Редактор конфигурации построен вокруг сущностей: слева всегда видна панель со списком всех сущностей (провайдеры, fallback, операции, fusion, router, оценка стоимости) и полем поиска по ней; выбор сущности в панели переключает тот же самый набор вкладок. Кнопки `Preview Changes`/`Save Configuration` и статус несохранённых изменений закреплены в нижнем sticky-футере, поэтому они всегда видны без прокрутки, даже если открытая сущность содержит много карточек.
...
- **OpenRouter Free**: read-only рейтинг бесплатных text-моделей OpenRouter. Lite eval включает instruction following, tool-call JSON, Python code с unit tests, параметризованную арифметику и grounded QA с отказом от галлюцинаций. Вкладка показывается только если настроен официальный провайдер `openrouter` и его API-ключ; правила маршрутизации она автоматически не меняет. Кнопка `Run Full Eval` запускает полноценную переоценку всех eligible моделей в фоне (`POST /v1/openrouter/free-models/run`) без ожидания следующего планового цикла.
...
- **Router**: структурированная форма для управления Router-моделями (`models_router_rules.json`): имя gateway-модели, selector-модель из уже настроенных chat fallback-моделей и явный список целей. Цель выбирается как полная gateway chat-модель или конкретная позиция в её fallback-цепочке. Там же задаются подсказки для селектора: политика маршрутизации у модели, описание и уровень стоимости (`free`/`cheap`/`standard`/`premium`) у каждой цели.
...
Все задания генерируются из seed прогона (`int(time) // 8 часов`), поэтому данные заданий меняются между прогонами и их нельзя заучить, а внутри одного прогона все модели получают одинаковые задания и сравнимы между собой. В `details` каждой задачи сохраняется обрезанный до 500 символов сырой ответ модели (`rawOutput`), чтобы разобрать провал без повторного платного прогона.

Lite eval даёт максимум 750 баллов и состоит из пяти задач:

- `instruction_following_lite` — максимум 200 баллов. Модель должна вернуть ровно 4 строки: `STATUS: <слово>`, строку с маркерным словом ровно N раз, JSON `{"mode":"<режим>","count":<число>}` и завершающее слово, без markdown и лишнего текста. Слова, число повторов, режим и count берутся из seed.
- `tool_call_lite` — максимум 200 баллов. Модель получает список доступных инструментов (порядок зависит от seed) и просьбу создать bug ticket. Оценивается, что она вернула JSON с инструментом `create_ticket` и правильными priority, assignee, due date и смыслом title — все эти значения генерируются из seed.
- `code_unit_lite` — максимум 200 баллов. Модель должна вернуть JSON с Python-функцией вида `sum_<фильтр>_<преобразование>(nums: list[int]) -> int` (например `sum_odd_cubes`); в задание входит сгенерированный пример вызова с ожидаемым результатом. Код проверяется на наличие, безопасный AST без импортов и опасных конструкций, затем запускается локальный unit test на сгенерированных входах.
...
- `grounded_qa_lite` — максимум 50 баллов. Модель получает три факта об отгрузках складов и два вопроса: ответ на первый есть в фактах, на второй — нет. Правильное число из фактов даёт 25 баллов, честный `UNKNOWN` вместо выдуманного числа — ещё 25. Задача ловит галлюцинации: выдуманное число записывается в `details.hallucinatedPallets`.

Во вкладках OpenRouter Free и Fallback Eval над списком моделей показывается сводка по задачам: сколько моделей прошло каждую задачу, средний процент баллов и три самые частые проваленные проверки поперёк всех моделей. У каждой модели раскрывающийся блок `Ответы моделей по тестам` показывает по каждой задаче её баллы, статус, список проваленных проверок и сырой ответ модели.
...
Отдельно различаются три вида `409`. Если другой клиент успел сохранить новую ревизию через API, это обычный конфликт ревизии: блок предлагает перезагрузить документ, и после перезагрузки сохранение проходит. Если же любой из шести конфигурационных файлов был изменён на диске мимо UI (ручная правка, деплой, скрипт) — включая тот, который вы сейчас редактируете, — сервер отвечает `409` с кодом `config_sources_out_of_sync` и перечисляет разъехавшиеся файлы: сохранение любого документа блокируется, потому что валидация опирается на все шесть файлов сразу. Обычная перезагрузка страницы здесь не помогает — `GET` отдаёт тот же снимок из памяти процесса. В этом случае блок конфликта в **Rules Editor** и в **Pricing** предлагает «Перечитать конфигурацию с диска», а его кнопка вызывает `POST /v1/config/resync` (master-only): gateway перечитывает все шесть файлов, валидирует их и публикует новое поколение runtime, ничего не записывая на диск. Если файлы на диске невалидны, resync отвечает `400` и текущая конфигурация остаётся работать; в ответе указывается, какой именно файл не прошёл валидацию (сам разбор Pydantic/json5 наружу не отдаётся, чтобы не раскрывать содержимое конфигурации), и редактор показывает это в подробностях ошибки. После успешного resync сохранение из редактора снова доступно, перезапуск сервиса не требуется. Изменение прав или владельца конфигурационного файла снаружи (`chmod`, `chown`) сохранению не мешает: запись сверяется с актуальным состоянием файла на диске, а новые права и владелец переносятся на записанный файл.

Отдельный случай — обновление кода, меняющее схему конфигурации. Если файл на диске уже записан в новом формате, а процесс всё ещё работает на старом коде, редактор встаёт в тупик: сохранение блокируется как `config_sources_out_of_sync`, а resync отвечает `400`, потому что запущенный процесс валидирует диск своей прежней схемой и не знает новых полей. Из UI это состояние не разрешается — resync намеренно не принимает конфигурацию, которую не может провалидировать. Лечится перезапуском сервиса (`systemctl restart llm-gateway.service` либо перезапуском контейнера).

Третий вид `409` — код `config_generation_busy`. Он означает, что на диске ничего не менялось и конфликта версий нет. Сохранение конфигурации **не ждёт** завершения запросов: новое поколение runtime публикуется сразу, новые запросы уходят по новой конфигурации, а уже начатые доигрывают на прежнем снимке и отпускают его, когда закончат. Пока запросы держат прежнее поколение, оно остаётся «уходящим» — его прокси-клиенты ещё не закрыты. Одновременно допускается до 4 таких уходящих поколений, и только при попытке публикации сверх этого лимита возвращается `409 config_generation_busy` (то есть после 4 сохранений подряд, ни одно из которых не успело освободиться из-за долгого web research или deep research). Перезагрузка документа здесь бессмысленна и только уничтожит правки, поэтому редактор её не предлагает: он показывает отдельную плашку «Шлюз ещё обслуживает запросы на прежней конфигурации» и сам повторяет тот же запрос с тем же `If-Match` — до 5 попыток с паузой 15 секунд. Пока идёт ожидание, доступна кнопка «Прекратить повторы». Как только долгие запросы завершились, повтор проходит успешно и сохранение публикуется без вмешательства оператора; если попытки исчерпаны, правки остаются в форме и достаточно нажать «Сохранить» ещё раз позже.
...
Router-модель — это обычная chat gateway-модель, которая перед основным вызовом выбирает одну цель из заранее заданного списка. Сначала она проверяет последние результаты инструментов: явная ошибка может сразу выбрать единственную `premium`-цель, а успешные тесты после изменения файлов — единственную `cheap`-цель. В остальных случаях вызывается отдельная selector-модель. `selector_model` должен быть gateway chat-моделью из `models_fallback_rules.json`, поэтому для самого selector сохраняется обычная fallback-цепочка. Внутренний selector-вызов не проверяется по `allowed_models` виртуального ключа, но внешний доступ к Router-модели и финальная выбранная gateway-цель проверяются как обычные chat-модели.
...
Селектору можно объяснить, по какому правилу выбирать. `routing_policy` на уровне Router-модели добавляется в системный промпт селектора, а у каждой цели можно задать `description` (для каких задач цель подходит) и `cost_hint` (`free`, `cheap`, `standard` или `premium`). Все три поля необязательны. Для прямого выбора Router сначала ищет единственную цель с нужным `cost_hint`; если такой подсказки нет, он может определить самую дешёвую или дорогую цель по уникальной сумме `input_rate + output_rate` среди целей без `cost_hint`. Нулевая цена и цель с `cost_hint: free` в прямом выборе не участвуют. Если однозначного кандидата нет, селектор получает весь список целей. Цены и лимиты подставляются из `providers.json` (`input_rate`, `output_rate`, `context_length`, `max_completion_tokens`, `upstream_limits`).
...
[
  {
    ...
    "routing_policy": "Prefer the cheapest candidate that can still answer well. Escalate only for code, tool calls, long context or multi-step reasoning.",
    "targets": [
      {
        "type": "gateway_model",
        "model": "llmgateway/high",
        "description": "Code and debugging, tool calls, long context, analysis",
        "cost_hint": "premium"
      },
...
### Overview
Страница http://localhost:9000/v1/ui/overview — стартовая страница после входа для обеих ролей (`/` редиректит сюда). Показывает: баннер статуса шлюза (`GET /health` — фаза `Starting`/`Ready`/`Stopping`/`Failed` и версия; отдельный текст «Status unavailable» показывается только при полной недоступности `/health`, например при сетевой ошибке, а не при обычном ответе с кодом `503`, у которого есть валидное тело отчёта — такой ответ показывается как «Degraded»), 6 приоритетных KPI (запросы, стоимость, p95 длительность, активные запросы, fallback-ошибки, отказы), один график тренда (запросы или стоимость по времени) и таблицу top-моделей по количеству запросов. Данные берутся из уже существующего `GET /v1/api/analytics-dashboard` с фильтром `range` (`24h`/`7d`/`30d`/`90d`); отдельного API для страницы не заводилось. Если за выбранный диапазон нет данных, вместо контента показывается явный empty-state с причиной.

### Activity
Страница http://localhost:9000/v1/ui/activity — единый журнал запросов шлюза, доступен обеим ролям. Список пагинируется через `GET /v1/api/usage-records`; фильтры: статус (`All` — 2xx/active, `Rejected (4xx)` — только master, работает через `GET /v1/admin/rejections`, `Server errors (5xx)` — см. ниже), `Since` (UTC, применяется только к виду Rejected), Provider, Key ID (только master) и Request ID — Provider/Key ID/Request ID фильтруют уже загруженную страницу без отдельного запроса. Колонки строки: время, request ID (копируется по клику), ключ (только master), операция, модель, провайдер, статус, длительность, стоимость. Вид **Server errors (5xx)** — заглушка: gateway пока не хранит отдельный журнал upstream 5xx-ошибок, поэтому вместо запроса к API страница сразу показывает дисклеймер «No data source for 5xx» без обращения к сети. Клик по строке открывает боковую панель (drawer) с деталями запроса: Request ID с кнопкой **Copy** (копирование через `navigator.clipboard.writeText` с fallback на `document.execCommand('copy')` и визуальным подтверждением «Copied!»), время в UTC и в локальном часовом поясе, статус (Success/Running/Rejected с цветовой кодировкой), виртуальный ключ — id и имя (только master, имя подгружается лениво через `GET /v1/admin/api-keys`), операция, запрошенная модель и фактически резолвленная модель, провайдер, prompt/completion токены, стоимость и длительность. Панель реализована как переиспользуемый компонент (`static/request-drawer.js` + `static/request-drawer.css`), с focus trap и закрытием по `Escape` (через существующий `createDialog`), фокус после закрытия возвращается на строку. Раздел **Fallback timeline** и поле **TTFT** сейчас показывают честный дисклеймер вместо данных: `GET /v1/api/usage-records` пока не отдаёт ни `ttft_ms`, ни журнал попыток fallback по конкретному запросу, поэтому эти данные не подставляются задним числом и не выдумываются. Все фильтры и текущая страница сериализуются в query string (`?status=...&provider=...&page=...`), поэтому обновление вкладки и ссылка «поделиться» восстанавливают тот же вид; если после успешной загрузки очередной запрос падает (сеть/5xx), таблица не очищается — остаётся последний загруженный снимок (кэшируется в `sessionStorage`) с бейджем «Stale · N мин назад» рядом со сводкой.

### Analytics (устар.)
На странице http://localhost:9000/v1/ui/usage-stats (только master) можно посмотреть подробную статистику использования; в навигации пункт подписан «Аналитика (устар.)», так как основной landing-страницей и основным журналом запросов теперь являются **Overview** и **Activity** выше, а эта страница сохранена как более детальная (но временная) альтернатива.
...
Дашборд также показывает расширенную аналитику:
- **Токены/с** — скорость генерации (completion-токены в секунду по строкам с известной длительностью) в KPI, в таблице resolved target и в разрезах по провайдерам.
- **Без failover (pin-honored)** — доля chat-запросов, обслуженных без единой неудачной upstream-попытки (по сохранённому в usage-записи полю `fallback_depth`). Считается только по запросам, для которых глубина fallback известна; операции без attempt-трейла в знаменатель не входят.
- **Показатели провайдеров** — отдельная таблица с per-provider скоростью генерации, средним TTFT, p95 длительности и слитой из fallback-событий статистикой попыток/ошибок/успешности; колонки `Ошибки fallback` и `Доля успешных` добавлены и в таблицу resolved target.
- **Счётчики за всё время** (только для master) — запросы, токены и стоимость с момента первого события. Они хранятся в отдельных таблицах `usage_hourly`/`usage_lifetime`, обновляются в той же транзакции, что и usage-запись, один раз бэкофиллятся из существующих строк при первом старте после обновления и переживают автоочистку сырых записей. График серии без активных фильтров строится по почасовым агрегатам, поэтому дальние диапазоны (`365d`, `12m`) показывают историю за пределами retention сырых строк; при любом активном фильтре серия считается по сырым строкам.
- **Client IP и User-Agent** — для каждой chat-usage-записи сохраняются IP пира и заголовок `User-Agent` клиента (обрезается до 256 символов); обе колонки видны в Recent Activity.
...
На странице `/v1/ui/api-keys` master-админ управляет виртуальными ключами и их лимитами. Таблица ключей загружается независимо от каталога моделей: `/v1/models` запрашивается только при первом открытии Create/Edit и кешируется до перезагрузки страницы. При ошибке диалог показывает явную кнопку повтора, а сохранённые `allowed_models`, которых больше нет в каталоге, остаются доступными для редактирования и не теряются при сохранении. Таблицу ключей можно фильтровать по названию/ID через поле поиска и сортировать по названию, дате создания или последнему использованию. Usage-статистика для master доступна как общий обзор на `/v1/ui/usage-stats`; per-key usage-фильтр для master не применяется.
...
- `PATCH /v1/admin/api-keys/{id}` — частичное обновление полей ключа, включая `reset_spent: true` для обнуления счётчика и `budget_period` для изменения периода сброса; ответ содержит обновлённые `budget_period` и `budget_reset_at`. Поддерживает опциональный заголовок `If-Match` со значением `ETag`, который возвращает `GET /v1/admin/api-keys/{id}`: при несовпадении ответ — `412 Precondition Failed` без применения изменений (защита от потери правок при одновременном редактировании ключа в двух вкладках); без `If-Match` поведение не меняется — сохранена обратная совместимость.
...
В верхней части страницы блок **быстрого старта** из четырёх карточек показывает первые шаги: где взять API-ключ (`GATEWAY_API_KEY` или виртуальный ключ через страницу **API-ключи**), пример первого запроса через `curl`, пример на Python SDK (`openai`-клиент с `base_url` gateway) и ссылку на страницу расхода.
...
Все одиннадцать примеров кода снабжены кнопкой копирования (иконка в правом верхнем углу блока временно переключается на «Скопировано!») и используют origin текущей страницы; оглавление раздела на экранах от 1024px закрепляется в правой колонке (`position: sticky`) и подсвечивает текущий раздел при прокрутке, на более узких экранах остаётся обычным блоком ссылок сверху; при открытии Docs через публичный HTTP интерфейс показывает предупреждение о незащищённой передаче API-ключей и запросов.
...
Секция **Chat** построена вокруг ленты сообщений (chat-first layout): форма отправки закреплена под лентой, а необязательные параметры (`top_p`, `max_tokens`) убраны в раскрывающийся блок «Дополнительные настройки». Обычные (не-Fusion) модели отправляются с `stream: true` и ответ ассистента дозаполняется по мере получения `data:`-чанков SSE; пока идёт стриминг, кнопка `Send` заблокирована и рядом появляется кнопка **Cancel**, обрывающая запрос через `AbortController` — уже полученный текст остаётся в пузыре с пометкой «(отменено)». В поле сообщения `Enter` отправляет чат, а `Shift+Enter` вставляет перенос строки. Селектор моделей секции **Chat** включает обычные fallback-модели, Fusion-ансамбли (`models_fusion_rules.json`) и Router-модели (`models_router_rules.json`); Fusion-модели помечены суффиксом `· Fusion` и, поскольку gateway отклоняет `stream: true` для Fusion, всегда отправляются как обычный буферизованный запрос. При выборе Fusion-модели появляется чекбокс **include_details** (по умолчанию включён) — он передаётся в запрос как `fusion.include_details`. Если в ответе есть поле `fusion`, под текстом ассистента раскрывается блок «Fusion ensemble» с атрибуцией main/judge, структурным разбором судьи (agreements/disputes/per-model insights/blind spots) и ответами панели (контент панели показывается только при включённом `include_details`, ошибочные панелисты помечаются отдельно). Router-модели вызываются как обычный chat, а подробности выбора возвращаются в поле `router` JSON-ответа.

Селектор **Тип моделей** в секции **Chat** переключает форму между моделями шлюза (поведение по умолчанию) и моделями провайдеров. В режиме «Модели провайдеров» вместо списка моделей шлюза появляются два списка: провайдер из `providers.json` и его модель, которая загружается по `GET /v1/config/providers/{provider}/models`. Такой запрос уходит выбранному провайдеру напрямую: gateway передаёт имя модели upstream как есть, не применяя model policy, правила Fusion/Router и цепочку fallback, поэтому ошибка провайдера показывается в чате как есть, без молчаливой подмены другой моделью. Если каталог моделей провайдера загрузить не удалось, список остаётся пустым и под формой выводится причина ошибки. Технически режим включается master-only заголовком `X-LLMGateway-Provider: <provider>` у обычного `POST /v1/chat/completions`, так что учёт токенов и стоимости работает как для остальных чат-запросов; виртуальные ключи получают на этот заголовок `403`, а неизвестный провайдер — `404`.
...
**Секция Editor** отображает таблицу (provider, model, input rate, output rate) с редактированием в ячейках. Кнопка «Add Row» добавляет новую строку, «Delete» удаляет, «Save Changes» записывает весь список в `providers.json` через PUT-запрос. Изменённые строки подсвечиваются. Запись атомарная; если файл содержал JSON5-комментарии — они сохраняются в резервную копию перед перезаписью. На мобильной ширине (<768px) в таблице остаются только Provider/Model — колонки ставок скрыты, кнопка «›» у строки открывает нижний лист с редактированием input/output rate для этой строки.

Таблица показывает не только модели, явно прописанные в `providers.json`, но и все модели, реально используемые конфигурацией (`models_fallback_rules.json`, `models_operation_rules.json`, `models_fusion_rules.json`, `models_router_rules.json`) и ещё не имеющие цены. Каждая строка помечена статусом `source`:
- `configured` — цена явно задана в `providers.json` (редактируемая строка, как раньше);
- `openrouter_autofill` — модель провайдера `openrouter` найдена в каталоге OpenRouter (`/api/v1/models`), цена подставлена автоматически и записана в `providers.json` (бейдж «Auto-filled from OpenRouter», строка остаётся редактируемой);
- `operation_default` — модель используется только служебной операцией (embeddings/rerank/images/audio/web/pdf) и ещё не имеет цены; строка нередактируема, показывает `$0.10 за запрос` (значение по умолчанию для операционных моделей, см. ниже) и кнопку «Set manual price» для перехода к ручному вводу;
- `upstream_only` — обычная (chat) модель без известной цены, стоимость которой учитывается только по данным upstream (`usage.cost`); строка тоже нередактируема до нажатия «Set manual price»;
- `awaiting_openrouter_catalog` — модель провайдера `openrouter`, каталог цен которого ещё не был загружен сервисом `OpenRouterFreeModelsService`; после первой успешной синхронизации строка автоматически переклассифицируется в `openrouter_autofill` или в один из статусов выше.

Автозаполнение выполняется на каждом GET-запросе к `/v1/admin/pricing`: если находятся цены для используемых моделей `openrouter`, они атомарно (CAS, с проверкой ревизии) дописываются в `providers.json`, без удаления уже настроенных цен других провайдеров.
...
- `GET /v1/admin/pricing` — список пар (provider, model), которые либо явно настроены в `providers.json`, либо реально используются конфигурацией; каждая запись содержит `source` (`configured`/`openrouter_autofill`/`operation_default`/`upstream_only`/`awaiting_openrouter_catalog`) и, для операционных моделей без цены, `default_cost_per_request`.
...
### Free LLM Catalog
Страница http://localhost:9000/v1/ui/free-models доступна обеим ролям (master и виртуальные ключи) и показывает каталог бесплатных LLM-моделей с [freellmapi.co](https://freellmapi.co/models), отфильтрованный по разумному месячному лимиту токенов.

- **URL**: `/v1/ui/free-models`
- **Источник**: `GET https://api.freellmapi.co/v1/latest` — открытый JSON без парсинга HTML, обновляется у источника примерно раз в сутки.
- **Критерий фильтра**: показываются только модели, у которых нижняя граница месячного токен-бюджета источника (например, `~50-100M` → 50M) не меньше порога — по умолчанию 30 000 000 токенов/мес, настраивается через `FREE_LLM_CATALOG_MIN_MONTHLY_TOKENS`. Модели с неизвестным бюджетом (`free · 40 RPM`, `promo (trial)` и т.п.) в список не попадают. Провайдер отображается, если у него осталась хотя бы одна модель.
- **Что показывает**: группировку по провайдерам (сортировка по максимальному бюджету провайдера), для каждой модели — название, `modelId` (моноширинный, с кнопкой копирования), бюджет-строку источника, заполненные лимиты (RPM/RPD/TPM/TPD — только те, что реально заданы), контекстное окно, бейджи vision/tools и раскрываемый блок «особенности подключения» (quirks).
- **Обновление данных**: фоновая задача забирает каталог при старте шлюза и затем раз в 24 часа; пока нет ни одного успешного снапшота, повтор происходит каждые 5 минут. Данные хранятся в памяти и не поштучно поллятся страницей — она делает один запрос при загрузке плюс повтор по кнопке.
- **Состояния**: пока не было ни одного успешного снапшота — «данные недоступны» с текстом последней ошибки; если снапшот есть, но последнее обновление упало — уже показанные данные остаются на экране с пометкой «по состоянию на \<дата\>» и текстом ошибки.
- **Мобильная ширина**: кнопки «Retry» и копирования `modelId` имеют высоту не менее 44px (сенсорный target).
- **Поиск и разделение «Настроенные/Внешние»**: строка поиска фильтрует модели по названию, `modelId` и имени провайдера (debounce 200мс); переключатель «Все / Настроенные / Внешние» доступен, когда удалось получить список настроенных моделей через `GET /v1/config/providers/structured` (master-роль — у виртуальных ключей этот запрос отдаёт 403, и каталог остаётся единым списком без бейджей, как раньше). При успешном запросе каталог делится на секции «Настроенные (N)» и «Внешние (M)» с бейджами у каждой модели.

API эндпоинт: `GET /v1/api/free-models` — отдаёт `{updatedAt, sourceVersion, sourceGeneratedAt, lastError, minMonthlyTokens, providers}`; доступен обеим ролям.
...
# Максимум дочерних операций одного deep research исследования
DEEP_RESEARCH_MAX_CHILDREN=1024
...
| `DEEP_RESEARCH_MAX_CHILDREN` | Максимум дочерних операций (`web_search`, `web_read`, `embeddings`, `chat`), которые одно исследование `POST /v1/web/deep-research` может выполнить за весь run. Считается суммарно, а не по параллелизму: их квитанции удерживаются до итогового rollup-события. Исчерпание лимита возвращает `503` с кодом `active_session_limit`. | `1024` |
...
| `FREE_LLM_CATALOG_ENABLED` | Включает фоновую задачу, которая забирает каталог бесплатных LLM-моделей с freellmapi.co для страницы `/v1/ui/free-models`. При `false` фоновый цикл не делает сетевых запросов, а API отдаёт пустой снапшот. | `true` |
| `CAPABILITY_AUTOFILL_ENABLED` | Включает фоновую задачу автозаполнения capability-полей (`supports_vision`/`supports_tools`/`context_window`) в правилах фаллбека по каталогам `/models` провайдеров. При `false` фоновый цикл не делает сетевых запросов; автозаполнение при сохранении в редакторе правил продолжает работать. | `true` |
| `FREE_LLM_CATALOG_MIN_MONTHLY_TOKENS` | Порог фильтра каталога бесплатных моделей: нижняя граница месячного токен-бюджета источника, ниже которой модель не попадает на страницу. | `30000000` |
...
| `YOUTUBE_PROXY_URL` | Прокси (`http://`, `https://`, `socks5://`, `socks5h://`) для скачивания субтитров YouTube в `/v1/web/read` и `/v1/web/research`. YouTube блокирует свой endpoint субтитров для IP дата-центров, поэтому при блокировке запрос без прокси завершается ошибкой `503` вместо подмены содержимым HTML-страницы видео. Когда переменная задана, запросы за субтитрами чередуются: один через прокси, следующий напрямую с IP gateway — так квота YouTube расходуется с двух адресов вместо одного. Если переменная не задана, все субтитры запрашиваются напрямую. Для SOCKS-прокси, резолвящих домены на своей стороне, указывайте схему `socks5h://`. | disabled |
| `YOUTUBE_FETCH_INTERVAL_SECONDS` | Пауза между двумя соседними скачиваниями субтитров YouTube. Все такие запросы проходят через общую очередь и выполняются строго по одному: пачка одновременных обращений мгновенно исчерпывает квоту YouTube сразу на обоих адресах, после чего блокировка держится часами. Значение — положительное число секунд. | `5` |
...
Поле `custom_headers` задаёт дополнительные HTTP-заголовки, которые gateway добавляет к **каждому** исходящему запросу к этому провайдеру: chat (включая прямой вызов из playground через `X-LLMGateway-Provider`), операции, список моделей и служебные проверки. Это нужно провайдерам, которые пускают только «свои» клиенты по заголовку `User-Agent`. Заголовки `Authorization`, `Cookie` и `X-Api-Key` здесь запрещены — ключ по-прежнему берётся только из `apikey` / `upstream_key_pools`. Если то же имя задано в `custom_headers` правила fallback или операции, значение правила перекрывает значение провайдера. В UI редактора поле находится в `Providers` → `Advanced options` → `Custom Headers` и вводится как JSON-объект.
...
[
    ...
    },
    {
        "customproxy":
        {
            ...
            // заголовки на каждый исходящий запрос, включая playground
...
Отдельный недоступный источник не срывает исследование. Если `web_read` вернул ошибку для конкретного URL (страница удалена, платный доступ, YouTube не отдал субтитры), этот URL исключается из выборки, а исследование продолжается на остальных источниках; каждый пропуск пишется в лог шлюза как ошибка callback с указанием URL и причины. Ошибки поиска (`web_search`) и генерации иллюстраций по-прежнему терминальны и завершают исследование с ошибкой — как и обрыв внутреннего канала между процессами: молча вернуть пустой отчёт вместо сломанного соединения шлюз не будет. Генерация итогового отчёта повторяется до 3 раз при сбое upstream, чтобы уже собранное исследование не выбрасывалось из-за одной неудачной попытки; ошибки внутреннего протокола между процессами повторов не получают и остаются терминальными.
...
>
> Pytest трактует deprecation warnings как ошибки. Единственное временное исключение — точный полный upstream warning `gpt-researcher==0.15.1` об импорте `langchain-community`; policy привязана ко всему message (включая URL и конец строки), category/module и автоматически истекает 2026-10-11 через dependency checker. Изменение текста или дополнительный suffix не подавляются. В W4 нужно повторно проверить upstream и удалить исключение, если импорт устранён. Остальные категории warnings не маскируются этим policy и разбираются в задачах соответствующих подсистем.
