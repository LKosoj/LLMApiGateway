...
Селектору можно объяснить, по какому правилу выбирать. `routing_policy` на уровне Router-модели добавляется в системный промпт селектора, а у каждой цели можно задать `description` (для каких задач цель подходит) и `cost_hint` (`free`, `cheap`, `standard` или `premium`). Все три поля необязательны: без них селектор получает только идентификаторы кандидатов и их fallback-цепочки, то есть выбирает без заданного критерия. Цены и лимиты моделей селектор видит отдельно — они подставляются из `providers.json` (`input_rate`, `output_rate`, `context_length`, `max_completion_tokens`, `upstream_limits`), поэтому `cost_hint` нужен как явная подсказка уровня, а не как замена реальных ставок.
