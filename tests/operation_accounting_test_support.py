...
def install_embeddings_rerank_accounting_passthrough(stack: ExitStack) -> None:
    """Keep legacy endpoint tests focused on HTTP behavior during atomic cutover."""
    owner = object()
    observation = object()
    ...
    stack.enter_context(
        patch.object(embeddings, "take_operation_terminal_owner", return_value=owner)
    )
    stack.enter_context(
    ...
    stack.enter_context(
        patch.object(embeddings, "finalize_buffered_operation", side_effect=finalize)
    )
    stack.enter_context(
        patch.object(embeddings, "release_operation_if_open", side_effect=release)
    )
    stack.enter_context(
        patch.object(
            embeddings,
