...
from ..inference import async_chat_completion_with_retry
...
class WebResearchClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._openai = AsyncOpenAI(api_key=openai_api_key, base_url=base_url or None)
    ...
    async def _call_openai_for_queries(self, user_prompt: str, system_prompt: str | None = None) -> list[str]:
        try:
            ...
            resp = await async_chat_completion_with_retry(
                client.chat.completions,
                operation="web_research_generate_queries",
                model=model,
                messages=messages,
                temperature=0.7,
                max_tokens=300,
    ...
    async def _extract_relevant_from_article_with_llm(self, user_query: str, article: dict[str, str]) -> str:
        ...
        resp = await async_chat_completion_with_retry(
            client.chat.completions,
            operation="web_research_synthesize",
            model=model,
            messages=[
...
def has_any_search_provider_config(
    *,
    ...
    return any(
        ...
        for value in (tavily_api_key, jina_api_key, zai_api_key, proxy_url)
    )
